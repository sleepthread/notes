...
那解决办法是什么呢？::标记::。标记用户信息和登录状态。

> 在学校或公司，入学入职那一天起，会录入你的身份、账户信息，然后给你发个卡，今后在园区内，你的门禁、打卡、消费都只需要刷这张卡。

**「前端存储」**

这就涉及到一发、一存、一带，发卡好办，后端接口生成并直接返回给前端，存储就需要前端想办法了。
...
![](https://i-blog.csdnimg.cn/blog_migrate/ce70cd3c73f26a2d0f0b8e8c34265dfd.png)
...
+   二是从「分布」角度，让相同 IP 的请求在负载均衡时都打到同一台机器上。以 nginx 为例，可以配置 ip\_hash 来实现。
...
![](https://i-blog.csdnimg.cn/blog_migrate/69881921e11d62970f9b75faf9fa5f7d.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/77bba491bdc50fb237419cf7fc8b659b.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/2fe24831e26dd2f8594b1b9c2ee1d599.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/859c51fdec7b3bb04fccd7db6f579ba4.png)

好了，现在 cdd 虽然能伪造出`eyJ1c2VyaWQiOiJhIn0=`，但伪造不出 sig 的内容，因为他不知道 secret。

**「JWT」**

但上面的做法额外增加了 cookie 数量，数据本身也没有规范的格式，所以 JSON Web Token Introduction - [http://jwt.io](https://link.zhihu.com/?target=http%3A//jwt.io "http://jwt.io") 横空出世了。
...
![](https://i-blog.csdnimg.cn/blog_migrate/9455a891fe7f141d2eac58c937e7a2f6.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/4a27e00b0ca53d8885dd9090e1638029.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/27ab5f3b32eaddb904f4b30daa7bfda1.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/04bc69fc138975b5d1e07b8810d21ca6.png)
...
![](https://i-blog.csdnimg.cn/blog_migrate/a85925c953fb434a42a5d29fb83e2193.png)

对浏览器来说，SSO 域下返回的数据要怎么存，才能在访问 A 的时候带上？浏览器对跨域有严格限制，cookie、localStorage 等方式都是有域限制的。

这就需要也只能由 A 提供 A 域下存储凭证的能力。一般我们是这么做的：

![](https://i-blog.csdnimg.cn/blog_migrate/87913d07ad88df64e9d4907b100e7717.png)
